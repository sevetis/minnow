...
void TCPSender::push( const TransmitFunction& transmit )
{
  ...
  while ( is_SYN || ( winsize_ != 0 && ( stream_closed || reader().bytes_buffered() > 0 ) ) ) {
    ...
    TCPSenderMessage msg = TCPSenderMessage { sent_ackno_,
                                              is_SYN,
                                              payload,
...
{
  return TCPSenderMessage { .seqno = sent_ackno_, .RST = reader().has_error() };
}
...
void TCPSender::receive( const TCPReceiverMessage& msg )
{
  ...
  if ( msg_abs_ackno > sent_abs_ackno )
    return;
  if ( msg_abs_ackno > received_abs_ackno && !in_flight_msg_.empty() ) {
    ...
    while ( !in_flight_msg_.empty()
            && msg_abs_ackno >= ( in_flight_msg_.front().seqno.unwrap( isn_, byte_popped )
